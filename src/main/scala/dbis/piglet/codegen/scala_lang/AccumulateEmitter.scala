...
/**
  ...
  */
class AccumulateEmitter extends CodeEmitter {
  ...
  override def code(ctx: CodeGenContext, node: PigOperator): String = {
    node match {
      case Accumulate(out, in, gen) => {
        val inputSchemaDefined = node.inputSchema.isDefined
        require(node.schema.isDefined)
        val outClassName = ScalaEmitter.schemaClassName(node.schema.get.className)
        ...
        var res = CodeEmitter.render(aggrTemplate, Map("out" -> node.outPipeName,
          "helper_class" -> helperClassName,
          "seq_expr" -> updExpr,
          "comp_expr" -> compExpr))
        res += "\n"
        res += render(Map("out" -> node.outPipeName,
          "in" -> node.inPipeName,
          "helper_class" -> helperClassName,
          "class" -> outClassName,
          "init_expr" -> initExpr,
