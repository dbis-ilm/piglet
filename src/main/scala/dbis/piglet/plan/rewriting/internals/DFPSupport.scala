...
import dbis.piglet.op.{Load, PigOperator, Pipe}
...
/** Provides methods for implementing some of [[dbis.piglet.plan.DataflowPlan]]s methods.
 *
 */
trait DFPSupport {
  ...
  /** Replace `old` with `repl` in `plan`.
    ...
    * @param old
    * @param repl
    * @return A new [[dbis.piglet.plan.DataflowPlan]] in which `old` has been replaced with `repl`.
    */
  //noinspection ScalaDocMissingParameterDescription
  ...
  /** Removes `rem` from `plan`.
    ...
    * @return A new [[dbis.piglet.plan.DataflowPlan]] without `rem`.
    */
  //noinspection ScalaDocMissingParameterDescription
