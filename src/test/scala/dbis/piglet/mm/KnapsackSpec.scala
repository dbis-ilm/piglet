...
class KnapsackSpec extends FlatSpec with Matchers with BeforeAndAfter with BeforeAndAfterAll {

  override def beforeAll: Unit = {
    val ps = new ProfilerSettings(minBenefit = 10.seconds,
      admissionCheck = false,
      cacheSize = 20,
      eviction = EvictionStrategy.KNAPSACK)
  ...
  "Knapsack" should "remove one element that does not fit from 2-elem cache" in {

    val newEntry = CacheEntry("new", "new", 1000L, 10, lastLoaded = Some(11))

    val list = Seq(
      CacheEntry("1", "1", _benefit=1000L, bytes=10, lastLoaded = Some(10)),
      CacheEntry("2", "2", _benefit=100L, bytes=10, lastLoaded = Some(10))
    )

    val wouldRemove = strategy.wouldRemove(newEntry, list)

    wouldRemove should contain only CacheEntry("2", "2", _benefit=100L, bytes=10, lastLoaded = Some(9))
  }
  ...
  it should "remove two entries from a 5-elem cache" in {
    ...
    val list = Seq(
      CacheEntry("1", "1", _benefit=1000L, bytes=4, lastLoaded = Some(11)),
      CacheEntry("2", "2", _benefit=100L, bytes=5, lastLoaded = Some(11)),
      CacheEntry("3", "3", _benefit=1000L, bytes=1, lastLoaded = Some(11)),
      CacheEntry("4", "4", _benefit=1000L, bytes=7, lastLoaded = Some(11)),
      CacheEntry("5", "5", _benefit=10L, bytes=3, lastLoaded = Some(11))
    )
  ...
  it should "remove all others if new entry fills cache alone" in {
    val newEntry = CacheEntry("new", "new", 2000L, 20, lastLoaded = Some(12))
